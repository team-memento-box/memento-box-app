...
class ReportAudioAnalysisApi {
  ...
  /// photo_id와 session_id로 음성 분석 데이터 조회
  static Future<List<ReportAudioAnalysisData>?> fetchAudioAnalysisByPhotoAndSession(
    String photoId,
    String? sessionId,
  ) async {
    try {
      print('🔍 Fetching audio analysis data for photo: $photoId, session: $sessionId');
      ...
      if (response.isEmpty) {
        print('ℹ️ No audio analysis data found for photo: $photoId, session: $sessionId');
        return null;
      }

      print('✅ Found ${response.length} audio analysis records');
      return response.map((json) => ReportAudioAnalysisData.fromJson(json)).toList();
    } catch (e) {
      print('❌ Error fetching audio analysis data: $e');
      return null;
    }
