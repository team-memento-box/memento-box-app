"""
16. 테스트하면서 미세조정
- fallback_threshold를 time_orientation이 못넘어서 예외처리
...
# System prompts
CASUAL_CHAT_SYSTEM_PROMPT = """당신은 7~15세 사이 호기심 많고 예의바른 어린이입니다. 
...
주의: 노인 사용자와 옛날 사진을 보며 대화하고 있습니다. 
사진 중심 대화는 과거의 주제에 집중하고, 현재의 주제는 피하세요.
history, 사진 메타데이터를 참고해 최대한 자연스러운 대화를 이어나가세요"""
...
TIME_ORIENTATION_TEMPLATE = "기억 여행을 시작합니다. 이 기록을 시작하는 오늘은 {current_year}년 9월 며칠인가요?"
TIME_TRAVEL_INTRO_TEMPLATE = "{years_ago}년이라는 시간을 거슬러 올라가는 중이에요…3..2..1"
...
ASSESSMENT_TASKS = {
    "registration": {
        ...
        "example_questions": [
            "사과, 배, 복숭아를 좋아했던 순서대로 말씀해주세요.",
            "파, 콩, 시금치를 싫어하는 순서대로 말씀해주시겠어요?",
            "아들, 큰딸, 작은 딸을 살가운 순서대로 말씀해주시겠어요?",
            "필통, 연필, 샤프를 어릴 적 갖고 싶었던 순서대로 말씀해주세요."
        ],
        "scoring_criteria": {
    ...
    "Naming": {
        ...
        주의: 물체가 있는 위치를 기반으로 그 사물이 무엇인지 질문해야 합니다.""",
...
class LangGraphDementiaChatbot:
    def __init__(self, config: ChatbotConfig):
        ...
        # Lightweight chat for fallback (gpt-5-nano only): 응답이 안와서 임시로 변경
    ...
    def check_and_score_assessment(self, state: ConversationState) -> ConversationState:
        ...
        print(f"[DEBUG] Assessment 답변 감지:")
        print(f"  - last_question_type: '{state.get('last_question_type', '없음')}'")
        print(f"  - is_assessment_answer (초기): {is_assessment_answer}")
        print(f"  - last_assessment_task (초기): '{last_assessment_task}'")
        ...
        if is_assessment_answer and last_assessment_task:
            ...
            # Add completed task to the set
            completed_tasks = state.get("completed_tasks", set())
            completed_tasks.add(last_assessment_task)
            print(f"[DEBUG] Task '{last_assessment_task}' 완료됨 - completed_tasks: {completed_tasks}")
        else:
            completed_tasks = state.get("completed_tasks", set())
    ...
    def _score_general_answer(self, user_answer: str, question: str, task_name: str) -> tuple:
        ...
        try:
            ...
            score_details = {
                ...
                "expected_answer": f"{task_name}",
                "score": score,
            }
            
            return score, score_details
        except Exception as e:
            # Return standard structure even for errors
            error_details = {
                "task_type": task_name,
                "assessment_question": question,
                "user_answer": user_answer,
    ...
    def _score_naming_answer(self, user_answer: str, question: str, photo_metadata: Dict) -> tuple:
        ...
        print(f"[DEBUG] Naming 채점 시작:")
        print(f"  - 질문: {question}")
        print(f"  - 사용자 답변: {user_answer}")
        print(f"  - Photo metadata 객체 수: {len(photo_metadata.get('objects', []))}, 사람 수: {len(photo_metadata.get('people', []))}")
        ...
        print(f"  - 추출된 타겟 객체: '{actual_target_object}'")
        
        if not actual_target_object:
            print(f"  - 타겟 객체 없음 - 일반 평가로 전환")
            # If no clear target, use general evaluation but mark as naming task
        ...
        try:
            ...
            print(f"  - 채점 LLM 응답: '{response.content}'")
            ...
            score_details = {
                ...
                "assessment_question": question,
                "user_answer": user_answer,
                "expected_answer": actual_target_object,
                "score": score,
            }
            
            print(f"[DEBUG] Naming 채점 완료: {score}/1.0")
            return score, score_details
        except Exception as e:
            # Return standard structure even for errors
            error_details = {
                ...
                "assessment_question": question,
                "user_answer": user_answer,
                "expected_answer": actual_target_object or "알 수 없음",
                "score": 0.0,
            }
    ...
    def _score_time_orientation_answer(self, user_answer: str) -> tuple:
        ...
        score = 0.0
        time_orientation_question = f"{CURRENT_YEAR}년 9월 며칠인가요?"
        expected_answer = f"{CURRENT_YEAR}년 9월 {today.day}일"
        ...
        if numbers:
            try:
                ...
                score_details = {
                    "task_type": "time_orientation",
                    "assessment_question": time_orientation_question,
                    "user_answer": user_answer,
    ...
    def _select_appropriate_descriptor(self, items: List[str], context: str = "") -> str:
        ...
        try:
            ...
            위 아이템들의 특성을 고려하여 가장 자연스러운 형용사를 하나만 반환해주세요.
    ...
    def evaluate_tasks_and_select(self, state: ConversationState) -> ConversationState:
        ...
        # Check if this is a time_orientation answer - if so, force casual mode for time travel transition
        if (state.get("is_assessment_answer", False) and 
            state.get("last_assessment_task") == "time_orientation"):
            return {
                **state,
        ...
        # Get already completed tasks from completed_tasks set
        completed_task_types = state.get("completed_tasks", set())
        
        print(f"[DEBUG] 완료된 task 체크:")
        print(f"  - completed_tasks: {completed_task_types}")
        ...
        if recall_scheduled_turn > 0 and current_turn >= recall_scheduled_turn and pending_recall_items:
            ...
            if is_first_session and "time_orientation" not in completed_task_types:
            ...
            elif is_first_session and "time_orientation" in completed_task_types:
                task_scores = self._evaluate_task_relevance(message, messages, photo_metadata)
                task_scores["time_orientation"] = 0.0  # Already completed
                task_scores["recall"] = 0.0  # No recall scheduled yet
                
                # Set already completed tasks to 0.0
                for completed_task in completed_task_types:
                    if completed_task in task_scores:
                        task_scores[completed_task] = 0.0
                ...
                # Set already completed tasks to 0.0 to prevent repetition
                for completed_task in completed_task_types:
                    if completed_task in task_scores:
                        task_scores[completed_task] = 0.0
...
숫자만 반환해주세요."""
        ...
        if selected_task == "time_orientation":
            return {
                **state,
                ...
                "last_assessment_task": selected_task
            }
    ...
    def _generate_casual_response(self, state: ConversationState) -> ConversationState:
        ...
        is_time_orientation_answer = (
            state.get("is_assessment_answer", False) and 
            state.get("last_assessment_task") == "time_orientation"
        )
        
        if is_time_orientation_answer:
        ...
        if use_lightweight:
            ...
            ai_response = self._lightweight_chat(current_message, photo_metadata, messages)
            response_type = "casual_lightweight"
        else:
            print("일반 casual 대화 모드")
            ai_response = self._standard_casual_chat(current_message, messages, photo_metadata)
    ...
    def _extract_date_from_message(self, message: str) -> tuple:
        ...
        # Unclear expressions
        unclear_patterns = [r'몰라|모르겠어|기억.*안.*나|잘.*모르겠어|확실하지.*않아']
    ...
    def _lightweight_chat(self, current_message: str, photo_metadata: Dict[str, Any] = None, messages: List = None) -> str:
        """Lightweight chat using gpt-5-nano"""
        try:
            context_info = ""
            
            # Add photo metadata as raw JSON
            if photo_metadata:
                context_info += f"\n\n사진 메타데이터: {json.dumps(photo_metadata, ensure_ascii=False, indent=2)}"
            
            # Add conversation history as raw JSON
            if messages:
                history_data = []
                for msg in messages:
                    if hasattr(msg, 'content'):
                        role = "human" if isinstance(msg, HumanMessage) else "ai"
                        history_data.append({"role": role, "content": msg.content})
                
                if history_data:
                    context_info += f"\n\n대화 히스토리: {json.dumps(history_data, ensure_ascii=False, indent=2)}"
            
            simple_message = f"{LIGHTWEIGHT_CHAT_PROMPT}{context_info}\n\n사용자 메시지: {current_message}\n\n자연스럽게 응답해주세요:"
    ...
    def _standard_casual_chat(self, current_message: str, messages: List, photo_metadata: Dict[str, Any] = None) -> str:
    ...
    def _extract_target_object_from_question(self, question: str, photo_metadata: Dict[str, Any]) -> str:
        ...
        print(f"[DEBUG] 타겟 객체 추출 시작:")
        try:
            all_objects = []
            
            # Extract from objects
            objects = photo_metadata.get("objects", [])
            print(f"  - Objects 배열: {objects}")
            for obj in objects:
                if isinstance(obj, dict):
                    obj_name = obj.get("name", "")
                    if obj_name:
            ...
            print(f"  - 추출된 모든 객체들: {all_objects}")
            
            if not all_objects:
                print(f"  - 사진에서 객체를 찾을 수 없음")
                return ""
            ...
            print(f"  - LLM에게 보내는 프롬프트: {system_msg}")
            response = self.llm.invoke([SystemMessage(content=system_msg)])
            target_object = response.content.strip().strip('"\'')
            print(f"  - LLM 응답: '{response.content}' -> 정제된 결과: '{target_object}'")
            
            if target_object in all_objects:
                print(f"  - 정확히 매칭됨: '{target_object}'")
                return target_object
            else:
                print(f"  - 정확히 매칭 안됨, 부분 매칭 시도...")
            ...
            print(f"[DEBUG] 질문에서 타겟 객체 추출 실패: {e}")
            return ""
    ...
    def chat_with_history(self, message: str, conversation_history: List[Dict[str, str]], photo_metadata: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_question_type": final_state.get("last_question_type", "")
        }
