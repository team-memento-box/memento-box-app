...
class ConversationState(TypedDict):
    ...
    conversation_mode: Literal["assessment", "casual", "fallback", "scoring"] 
    ai_response: str                       
    response_type: str                     
    workflow_stage: str                    
    retry_count: int                       
    ...
    assessment_feedback: str               # 채점 결과 피드백
...
ASSESSMENT_TASKS = {
    "registration_recall": {
        ...
        "scoring_criteria": {
            ...
            "scoring_type": "keyword_count"  # 채점 방식
        }
    },
    "Naming": {
        "description": 
        ...
        "scoring_criteria": {
            ...
            "scoring_type": "object_naming"  # 채점 방식
        }
    },
    "time_orientation": {
        "description": 
        ...
        "scoring_criteria": {
            "scoring_type": "date_accuracy"  # 채점 방식
        }
    }
}
...
class LangGraphDementiaChatbot:
    ...
    def _build_graph(self) -> StateGraph:
        ...
        workflow.add_node("provide_assessment_feedback", self.provide_assessment_feedback)
        ...
        workflow.add_edge("score_assessment_answer", "provide_assessment_feedback")
        workflow.add_edge("provide_assessment_feedback", END)
    ...
    def check_if_assessment_answer(self, state: ConversationState) -> ConversationState:
        """현재 메시지가 assessment 질문에 대한 답변인지 확인"""
        print("답변 확인: Assessment 질문에 대한 답변인지 체크...")
        
        messages = state["messages"]
        current_message = state["current_message"]
        
        # 대화 히스토리에서 마지막 AI 메시지가 assessment 질문인지 확인
        last_ai_message = ""
        ...
        if last_ai_message:
            # 각 태스크의 예시 질문들과 유사도 검사
            for task_name, task_info in ASSESSMENT_TASKS.items():
                example_questions = task_info["example_questions"]
                
                try:
                    all_texts = [last_ai_message] + example_questions
                    tfidf_matrix = self.vectorizer.fit_transform(all_texts)
                    last_message_vector = tfidf_matrix[0:1]
                    example_vectors = tfidf_matrix[1:]
                    
                    similarity_scores = cosine_similarity(last_message_vector, example_vectors).flatten()
                    max_similarity = float(np.max(similarity_scores))
                    
                    # 높은 유사도면 assessment 질문으로 판단
                    if max_similarity > 0.6:  # 임계값
                        is_assessment_answer = True
                        last_assessment_task = task_name
                        print(f"Assessment 답변 감지! Task: {task_name}, 유사도: {max_similarity:.2f}")
                        break
    ...
    def score_assessment_answer(self, state: ConversationState) -> ConversationState:
        ...
        try:
            ...
            elif scoring_type == "object_naming":
                # 사물 이름 맞추기 채점 (Naming)
...
0.0부터 1.0 사이의 점수만 반환해주세요."""
            ...
            elif scoring_type == "date_accuracy":
...
평가 기준:
- 질문에 직접적으로 답변함: 1.0점
- 부분적으로 답변하지만 불완전: 0.5-0.8점
- 질문과 관련없는 답변: 0.0-0.3점
...
0.0부터 1.0 사이의 점수만 반환해주세요."""
        ...
        return {
            **state,
            ...
            "conversation_mode": "scoring"
        }

    def provide_assessment_feedback(self, state: ConversationState) -> ConversationState:
        """채점 결과를 바탕으로 피드백 제공"""
        print("피드백 생성 중...")
        ...
        # 점수에 따른 피드백 생성
        if score >= 0.8:
            feedback_tone = "칭찬과 격려"
        elif score >= 0.5:
            feedback_tone = "부분적 인정과 격려"
        else:
            feedback_tone = "따뜻한 격려와 재시도 유도"
        
        system_msg = f"""당신은 7~15세 사이의 따뜻하고 격려하는 어린이입니다.
할머니/할아버지가 질문에 답변하셨는데, 채점 결과를 바탕으로 자연스러운 피드백을 해주세요.
...
피드백 예시:
- 높은 점수: "와! 정말 잘 기억하고 계시네요! 대단해요!"
- 중간 점수: "좋아요! 거의 다 맞으셨어요. 조금만 더 생각해보시면 될 것 같아요."
- 낮은 점수: "괜찮아요! 어려운 질문이었어요. 함께 다른 얘기해볼까요?"

자연스럽고 따뜻한 대화체로 피드백해주세요. 점수나 채점 결과를 직접 언급하지 마세요."""
        ...
        try:
            ...
            if score >= 0.8:
                ...
                feedback = "좋네요! 잘 생각하고 계세요."
            else:
                feedback = "괜찮아요! 다른 얘기해볼까요?"
        
        return {
            **state,
            "ai_response": feedback,
            "response_type": "assessment_feedback",
            "workflow_stage": "assessment_feedback",
...
if __name__ == "__main__":
    ...
    # 테스트 2: Assessment 답변 채점
    print(f"\n{'=== 테스트 2: Assessment 답변 채점 ===':=^80}")
    ...
    print(f"채점 점수: {result2['assessment_score']:.2f}/1.0")
    print(f"AI 피드백: {result2['ai_response']}")
    print(f"채점 상세: {result2['score_details']}")
    
    # 테스트 3: 날짜 질문 답변 채점
    print(f"\n{'=== 테스트 3: 날짜 질문 답변 채점 ===':=^80}")
    conversation_history_3 = [
        {"role": "user", "content": "오늘이 며칠인지 궁금해요."},
        {"role": "assistant", "content": "오늘은 몇일 인가요?"},
    ]
    
    # 오늘 날짜로 답변 (실제 날짜와 비교)
    from datetime import datetime
    today = datetime.now().day
    date_answer = f"오늘은 {today}일이에요."
    result3 = chatbot.chat_with_history(date_answer, conversation_history_3)
    
    print(f"\n{'테스트 3 결과':=^80}")
    print(f"메시지: '{result3['user_message']}'")
    print(f"Assessment 답변 감지: {result3['is_assessment_answer']}")
    print(f"채점 점수: {result3['assessment_score']:.2f}/1.0")
    print(f"AI 피드백: {result3['ai_response']}")
    print(f"채점 상세: {result3['score_details']}")
    
    # 테스트 4: 사물 이름 질문 답변 채점
    print(f"\n{'=== 테스트 4: 사물 이름 질문 답변 채점 ===':=^80}")
    conversation_history_4 = [
        {"role": "user", "content": "사진 속 가구가 궁금해요."},
        {"role": "assistant", "content": "케이크 밑에 있는 가구 이름은 뭔가요?"},
    ]
    
    object_answer = "테이블이에요."
    result4 = chatbot.chat_with_history(object_answer, conversation_history_4)
    
    print(f"\n{'테스트 4 결과':=^80}")
    print(f"메시지: '{result4['user_message']}'")
    print(f"Assessment 답변 감지: {result4['is_assessment_answer']}")
    print(f"채점 점수: {result4['assessment_score']:.2f}/1.0")
    print(f"AI 피드백: {result4['ai_response']}")
    print(f"채점 상세: {result4['score_details']}")
